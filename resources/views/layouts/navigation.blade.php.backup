<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    @can('viewAny', App\Models\Lokasi::class)
                    <x-nav-link :href="route('locations.index')" :active="request()->routeIs('locations.*')">
                        {{ __('Lokasi') }}
                    </x-nav-link>
                    @endcan

                    @can('viewAny', App\Models\Kelompok::class)
                    ...
                    <x-nav-link :href="route('logbooks.index')" :active="request()->routeIs('logbooks.*')">
                        {{ __('Logbook') }}
                    </x-nav-link>
                    @endcan
                ...
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        <div class="px-4 py-2 text-sm text-gray-700">
                            ...
                            @if(Auth::user()->nim)
                                <div class="text-gray-500">NIM: {{ Auth::user()->nim }}</div>
                            @endif
                            @if(Auth::user()->nip)
                                <div class="text-gray-500">NIP: {{ Auth::user()->nip }}</div>
                            @endif
                        </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @can('viewAny', App\Models\Logbook::class)
            <x-responsive-nav-link :href="route('logbooks.index')" :active="request()->routeIs('logbooks.*')">
                {{ __('Logbook') }}
            </x-responsive-nav-link>
            @endcan
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4">
                ...
                @if(Auth::user()->nim)
                    <div class="font-medium text-sm text-gray-500">NIM: {{ Auth::user()->nim }}</div>
                @endif
                @if(Auth::user()->nip)
                    <div class="font-medium text-sm text-gray-500">NIP: {{ Auth::user()->nip }}</div>
                @endif
            </div>
