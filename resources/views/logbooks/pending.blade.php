        ...
        <div class="row mb-4">
            <div class="col-md-12">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <h2 class="fw-bold">Validasi Logbook Pending</h2>
                        <p class="text-muted">Daftar logbook mahasiswa bimbingan yang menunggu validasi</p>
                    </div>
                    <div>
            ...
            <div class="col-md-12">
                ...
                <div class="card mb-4">
                    <div class="card-header" style="background-color: #f8f9fa;">
                        <h6 class="card-title mb-0" style="color: #0B1F3A;">
                            <i class="fas fa-filter me-2"></i>Filter Logbook
                        </h6>
                    </div>
                    <div class="card-body">
                    ...
                    <div class="card-header" style="background-color: #f2b70d;">
                        <div class="d-flex justify-content-between align-items-center">
                            <h5 class="card-title mb-0" style="color: #0B1F3A;">
                                <i class="fas fa-book me-2"></i>Logbook Pending
                                @if(request('nama') || request('jenis') || request('jurusan'))
                                    <small class="text-muted">({{ $logbooks->total() }} hasil)</small>
                                @endif
                            </h5>
                            @if($logbooks->count() > 0)
                                <div class="d-flex gap-2">
                                    <form action="{{ route('logbooks.approveAll') }}" method="POST" onsubmit="return confirm('Setujui semua logbook pending?')">
                                        @csrf
                                        <button type="submit" class="btn btn-success">
                                            <i class="fas fa-check-double me-1"></i>Setujui Semua
                                        </button>
                                    </form>
                                    <form action="{{ route('logbooks.rejectAll') }}" method="POST" onsubmit="return confirm('Tolak semua logbook pending?')">
                                        @csrf
                    ...
                    <div class="card-body">
                                    ...
                                    <tbody>
                                        @foreach($logbooks as $logbook)
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{ route('logbooks.show', $logbook) }}" class="btn btn-sm btn-info text-white mb-1">
                        ...
                        @else
                            <div class="text-center py-5">
                                ...
                                <h4 class="mt-3">Tidak Ada Logbook Pending</h4>
                                <p class="text-muted">Semua logbook mahasiswa bimbingan Anda sudah divalidasi</p>
                            </div>
                        @endif
                    </div>
                </div>
